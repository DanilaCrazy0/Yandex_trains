with open('02.txt', 'r') as f:
    sts = list(map(lambda x: x.strip(), f.readlines()))
    n = int(sts[0])
    lst = [int(x) for x in sts[1].split()]
    k = int(sts[2])
    inp = sts[3:].copy()

with open('02.a', 'r') as f:
    ans = list(map(int, f.read().split()))
...
print(lst)
for output, answer in zip(out, ans):
    if output - answer != 0:
        print(output - answer)
